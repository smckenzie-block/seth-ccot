...
- **`template_catalog.json`** — A structured catalog of all 51 current (2026) regulatory response templates, organized into 14 categories with keyword matching rules, Google Doc IDs, and usage guidance for each template.
- **`fetch_template.py`** — A Python helper script that fetches a Google Doc template's text content by its document ID using the Google Drive API. Used for reading template structure during drafting. Handles OAuth token refresh automatically via the macOS keychain.
- **`create_formatted_draft.py`** — A Python helper script that creates a fully formatted Google Doc draft by copying the selected template and replacing ZZZ placeholders in-place. This preserves all original template formatting: Cash App logo, Montserrat font, bold/italic styling, margins, footnotes, headers, and footers. The handler receives a ready-to-review Google Doc link with zero manual formatting required.
...
2. Retrieve the issue fields:
   ...
   - `customfield_11567` — Google Drive Link (**required for draft placement** — this is the complaint's dedicated folder where the formatted draft will be saved)
...
## Step 4: Create the Formatted Google Doc Draft

Instead of presenting the draft as plain text (which the handler would need to manually copy and format), this skill creates a **fully formatted Google Doc** that preserves the template's exact design.

### How It Works

1. **Copy the template.** The skill copies the selected Google Doc template using the Google Drive API. This preserves ALL formatting:
   - Cash App logo in the first-page header
   - Montserrat font family (11pt body, 10pt footnotes)
   - Bold/italic/underline styling on each text element
   - Page margins (63pt left/right, 93.6pt top, 90pt bottom)
   - Line spacing and paragraph indentation
   - Footer (1955 Broadway, Suite 600 / Oakland, CA 94612)
   - Footnotes with approved legal definitions
   - Section headers (I. Introduction, II. Your Complaint, III. Our Investigation, IV. Conclusion)

2. **Replace ZZZ placeholders.** Using the Google Docs `replaceAllText` API, the skill replaces all `ZZZ...ZZZ` placeholder tokens with the actual content from the investigation notes. The replacement text **inherits the exact formatting** of the placeholder it replaces — so if `ZZZNameZZZ` was bold 11pt Montserrat, "Jane Doe" will be bold 11pt Montserrat.

3. **Handle optional sections.** For `ZZZZ Option A / Option B ZZZZ` blocks, the skill removes the inapplicable option and strips the `ZZZZ` markers from the kept option.

4. **Place the draft in the complaint's Google Drive folder.** The draft must be saved directly into the complaint's dedicated Google Drive folder so it lives alongside all other case documents. The folder URL comes from the JIRA issue's Google Drive Link field (`customfield_11567`). The skill extracts the folder ID from that URL and passes it to the helper script via `--folder-url`. The script verifies write access before copying. If the JIRA field is empty or the folder is inaccessible, the skill must ask the handler for the correct folder link before proceeding — do NOT default to My Drive.

### Document Naming Convention

The draft Google Doc must be named using the **JIRA summary** (which follows the CF1 Doc Title convention). This is the standard naming convention used across the team:

**Draft document name:** Identical to the JIRA summary field.
```
[Agency] [YYMMDD]-[ComplaintID] [FirstInitial] [MiddleInitial(s)] [LastName]
```

**Examples:**
- `CFPB 260122-28077945 J Marenco`
- `CFPB 260310-29778459 D Wilkerson`
- `NY AG 260115-12345678 J J Schmidt`

**Where the name comes from:**
1. Parse the JIRA `summary` field (extracted in Step 1). This already follows the correct format.
2. Use the summary as-is for the `document_title` in the replacements JSON.

**After the handler finalizes the response** (not done by this skill — done manually by the handler during the Send step):
- The handler appends `_FINAL RESPONSE` to the document name.
- Example: `CFPB 260122-28077945 J Marenco_FINAL RESPONSE`
- For multiple responses to the same complainant: `_FINAL RESPONSE 1`, `_FINAL RESPONSE 2`, etc.

**Important:** The skill creates the draft with the base name only. It does NOT append `_FINAL RESPONSE` — that is added by the handler after review and approval, during the Send step (WID Section 1.3.1).

### Execution

To create the formatted draft, the skill:

1. Writes a temporary JSON file with all replacements:
   ```json
   {
     "document_title": "CFPB 260122-28077945 J Marenco",
     "destination_folder_url": "https://drive.google.com/drive/folders/...",
     "replacements": {
...
2. Runs the helper script:
   ```
   python3 <skill_directory>/create_formatted_draft.py <TEMPLATE_DOC_ID> <REPLACEMENTS_JSON> --folder-url "<GOOGLE_DRIVE_FOLDER_URL>"
   ```
   Where `<GOOGLE_DRIVE_FOLDER_URL>` is the value of `customfield_11567` from the JIRA issue.

3. The script outputs the URL of the new draft document.

### Presenting the Draft to the Handler

After creating the formatted Google Doc:

1. **Share the Google Doc link** with the handler. The document is ready to review — no manual formatting needed.
2. **Show which template was used** and how it was selected (auto-selected or handler-provided).
3. **List any `[HANDLER: ...]` markers** that need the handler's attention (missing information, ambiguous facts, etc.).
4. **Flag if PII was NOT VERIFIED** — this affects whether the response can include account-specific details.
...
7. **Remind the handler:** "To run a Legal and Compliance risk review on this draft, use the `ccot-compliance-reviewer` skill."
8. **Remind the handler** to review the Handler Post-Draft Checklist below before finalizing.

### Fallback: Plain Text Output

If the Google Drive API is unavailable (token expired, network error, permissions issue), fall back to presenting the draft as formatted text in the chat. The handler will need to manually copy it into a Google Doc and apply formatting. In this case, include a note:
> "⚠️ I was unable to create a formatted Google Doc automatically. Please copy the draft below into a Google Doc using the template formatting (Montserrat 11pt, bold section headers). The template is: [Template Name] — [Template URL].""
