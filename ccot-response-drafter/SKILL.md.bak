...
description: CCOT Response Drafter — Draft a Formal Complaint Response letter for the Cash Complaint Operations Team (CCOT) using JIRA investigation notes and a Google Document template
---

# CCOT Response Drafter

This skill helps CCOT complaint handlers draft a Formal Complaint Response — the final written letter sent to a consumer in reply to a regulatory complaint (e.g., CFPB). The handler provides a JIRA link (containing investigation notes) and a link to the appropriate Google Document template. The skill extracts the investigation findings and produces a complete draft response that follows the team's established format, tone, and style guide.
...
- "Write the complaint response using this JIRA and this template"
...
2. **Google Document Template Link** — a link to the appropriate response template from the CCOT template library
...
1. Open the provided Google Document template link using the Google Drive extension.
2. If the Google Drive extension is unavailable, ask the handler to paste the template text or provide a downloaded copy.
3. Identify the template type from the filename or content (e.g., "P2P - Cancel Authorized Payment", "Cash App Card - Dispute - Claim Resolved", "Account Closed - Denylisted", etc.).
...
2. Flag any areas where information was missing or unclear from the investigation notes (use `[HANDLER: ...]` markers).
3. Flag if the PII was NOT VERIFIED — this affects whether the response can include account-specific details.
4. Note the customer's mailing address — this must be provided by the handler if not in the investigation notes (it comes from the complaint filing, not JIRA).
5. After presenting the draft, remind the handler: "To run a Legal and Compliance risk review on this draft, use the `ccot-compliance-reviewer` skill."
...
- [ ] Review all facts against the investigation notes
- [ ] Verify the customer's full name and mailing address
- [ ] Confirm the response aligns with any SME/lead guidance from escalations
- [ ] Ensure the document uses **Montserrat font, Normal weight, 11pt** (body) and **10pt** (footnotes)
